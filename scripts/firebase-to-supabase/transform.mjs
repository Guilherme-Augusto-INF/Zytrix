...
const userId = firebaseUid => {
  ...
  const value = typeof mapped === 'string' ? mapped : mapped?.supabase_user_id;
...
for (const [firebaseUid, mapping] of Object.entries(authMap)) {
  const supabaseUserId = typeof mapping === 'string' ? mapping : mapping?.supabase_user_id;
  if (!supabaseUserId) throw new Error(`Invalid Auth mapping for ${firebaseUid}`);
