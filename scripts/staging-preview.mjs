...
http.createServer(async(req,res)=>{
 ...
 try{
  ...
  // Only the isolated validation page and public assets are served. Never expose private files or the legacy application.
  const path=pathname==='/'?'/staging-validation.html':pathname;
  if(path!=='/staging-validation.html'&&!path.startsWith('/assets/')){res.writeHead(404);res.end();return;}
  const file=resolve(root,'.'+decodeURIComponent(path));if(!file.startsWith(root+sep)||!mime[extname(file)]||!(await stat(file)).isFile()){res.writeHead(404);res.end();return;}
