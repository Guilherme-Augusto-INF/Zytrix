export function createPlatformClient(getUser) {
  return async function platform(action,data={}) {
    ...
    const response=await fetch('/api/v1/platform',{method:'POST',headers:{'Content-Type':'application/json',...(token?{Authorization:`Bearer ${token}`}:{})},body:JSON.stringify({action,data}),credentials:'same-origin',cache:'no-store',signal:AbortSignal.timeout(15000)});
